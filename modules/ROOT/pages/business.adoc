...
Further modules are added as functionality expands
...
The Change Management process is based on generic change and release management processes in https://en.wikipedia.org/wiki/ITIL[ITILv3] and the generic xref:new_main@EPO::metgovman.adoc[Methodology and tools for Structural Metadata Management and Governance]
...
* #The ontology is provided in UML, and a code list is recommended, but in OWL generation,there is no code list so users can use what they want.#
