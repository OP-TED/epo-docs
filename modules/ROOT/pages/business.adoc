...
:page-aliases: 4.2.0@business.adoc,4.2@business.adoc
endif::[]
////
...
More detail on the proposal for each release, go to the xref:epo-home::history.adoc[History and Future Developments page].
...
The Change Management process is based on generic change and release management processes in https://en.wikipedia.org/wiki/ITIL[ITILv3] and the generic xref:epo-home::REFmetgovman.adoc[Methodology and tools for Structural Metadata Management and Governance]
...
As of version 4.0.0, the data type rdf:PlainLiteral is used instead of rdfs:Literal.
