...
The formal ontology restrictions are divided into four variants. Three variants must be generated for each of the OWL 2 sublanguages. The transformation rules that require more complex statements must be omitted when generating these (simpler) variants.
...
Moreover, the eProcurement URI scheme must subscribe to the “Cool URIs” recommendations [17] and ensure that they don’t change [11].
...
|Root reference
a|{baseVoc}/ontology/{ontologyName}

|Document reference
a|{baseVoc}/ontology/{ontologyName}[#{documentRef}]

|Resources reference
a|{baseVoc}/ontology/{ontologyName}#{resourceName}
|===
...
|Root reference
a|{baseVoc}/reference/{listName}

|Document reference
a|{baseVoc}/reference/{listName}[#{documentRef}]

|Concept scheme
a|{baseVoc}/reference/{listName}

|Concepts
a|{baseVoc}/reference/{listName}#{concept}
|===
...
|Root reference
a|{baseVoc}/shape/{ontologyName}

|Document reference
a|{baseVoc}/shape/{ontologyName}#{documentRef}

|Shape resources
a|{baseVoc}/shape/{ontologyName}#{resourceName}
|===
...
|Document reference
a|{baseVoc}/rule/{ontologyName}#{documentRef}
...
|Root reference
a|{baseData}/resource{documentRef}

|Document reference
a|{baseData}/resource/{documentRef}/{documentRef}

|Fragment reference
a|{baseData}/resource/{documentRef}/{fragmentRef}[/{subFragmentRef}]

|Resources
a|{baseData}/resource/{documentRef}#{resourceId}
|===
