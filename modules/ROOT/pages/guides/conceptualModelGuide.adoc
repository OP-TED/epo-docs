...
As seen in figure 11, An epo:Agent can either be a person (foaf:Person) an organization (org:Organization) or a system (epo:system). Classes foaf:Person and org:Organization are themselves generalizations of more specific concepts. The prefixes foaf and org signify that these classes were reused by ePO from other ontologies and taxonomies.
